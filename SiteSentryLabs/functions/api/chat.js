/**
 * POST /api/chat — Sohana AI assistant.
 * Port of server/controllers/chatController.js: factual system prompt,
 * optional OpenAI-compatible provider (AI_API_URL / AI_API_KEY / AI_MODEL),
 * and the local keyword fallback when no provider is configured.
 */
import { json, clean, clientIp } from '../_shared.js';

const MAX_MESSAGE_LENGTH = 800;
const MAX_HISTORY = 10;
const RATE_WINDOW_MS = 60 * 1000;
const RATE_LIMIT = 30;
const buckets = new Map(); // best-effort, per isolate
...
Navigation: when useful, offer links/buttons to about.html, services.html, team.html or contact.html.
...
function limitedHistory(raw) {
  ...
  return raw
    ...
    .map((item) => ({
      ...
      content: clean(item && item.content, MAX_MESSAGE_LENGTH)
    }))
...
function limitedProjects(raw) {
  ...
  return raw
    .slice(0, 8)
    .map((p) => ({
      title: clean(p && p.title, 120),
      category: clean(p && p.category, 80),
      description: clean(p && p.description, 400)
    }))
    .filter((p) => p.title);
}
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (message.length > MAX_MESSAGE_LENGTH) {
      return json({ ok: false, error: `Message must be ${MAX_MESSAGE_LENGTH} characters or fewer.` }, 400);
    }

    const history = limitedHistory(body.history);
    const projects = limitedProjects(body.projects);
    const providerUrl = clean(env.AI_API_URL, 500);
    const providerKey = clean(env.AI_API_KEY, 500);
    const providerModel = clean(env.AI_MODEL, 100);
    ...
    if (providerUrl && providerKey && providerModel) {
      try {
        ...
        if (providerResponse.ok) {
          ...
          if (reply) return json({ ok: true, reply: clean(reply, 2500), mode: 'ai' });
        }
    ...
    return json({ ok: true, ...localReply(message, projects, history), mode: 'local' });
  } catch (error) {
    console.error('Sohana chat error:', error);
    return json({ ok: false, error: 'Unable to process your message right now.' }, 500);
  }
}
